...
public class IssueUpdater {
  ...
  public SearchResponseData saveIssueAndPreloadSearchResponseData(DbSession dbSession, DefaultIssue issue, IssueChangeContext context,
    ...
    QGChangeEventFactory.IssueChangeData issueChangeData = new QGChangeEventFactory.IssueChangeData(
      result.getIssues().stream().map(IssueDto::toDefaultIssue).collect(MoreCollectors.toList(result.getIssues().size())),
      copyOf(result.getComponents()));

    if (refreshMeasures) {
      issueChangePostProcessor.process(dbSession, issueChangeData, singleton(component));
    }
